...
def reportProgress(iteration, total):
    printProgressBar(
        iteration, total, prefix="Import", length=50, suffix="Complete", printEnd="\r\n"
    )
...
def importToTypeSense():
    ...
    res = input(
        ...
        + str(nb_products_to_import)
        + " produits dans TypeSense ? (yN) "
    )
    ...
    if res == "y":
        projection = {
            ...
            "product_name": 2,
            "countries": 3,
            "nutriments.carbohydrates_100g": 4,
        }
        ...
        while nb_products_imported < nb_products_to_import:
            batch_size = min(
                max_batch_size, nb_products_to_import - nb_products_imported
            )
...
def queryTypeSense(query, query_field="name"):
    search_params = {
        "q": query,
    ...
    return [hit["document"] for hit in res["hits"]] if res["hits"] else None
